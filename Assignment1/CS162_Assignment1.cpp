...
//CS 162
//Assignment 1
//This program will play against another player (a human) at a game of pick up sticks. The game will start by asking
//the user if he/she would like to read the instructions on how to play the game first. The user will then be prompted
//to enter the number of sticks the game should start with (an integer between 1 and 21, exclusive). The program then ask
//the user to select a head or tail and does a coin flip by using a random number generator. If the user selected correct,
//he/she starts the game. If not, the computer proceeds to remove sticks from the total sticks. The user and the computer
//takes turn to remove either 1 or 2 sticks from the initial total. The game ends when the next player is left with 1
//stick, at which point, that player loses. Through some deductions and mathematics, we find that there are certain
//number of sticks that if a player is stuck with, that player will lose against a skilled player. Those numbers include
//4. A player left with 4 sticks will lose no matter how many sticks he/she removes. The next number is 7, continuing
//on to 10, 13, 17, and 20. We call these the "poison" numbers. If the computer is left with these numbers, it will remove
//1 stick, since it does not matter how many sticks it removes. Furthermore, removing only 1 stick leaves the total sticks
//higher and an unskilled user will have more room to make mistakes. Otherwise, it will remove sticks so that the user is
//left with a "poison" number of sticks (i.e. if there are 6 sticks left, it will remove 2 sticks, or removes 1 stick if
//it is left with 5 sticks, and so on).
